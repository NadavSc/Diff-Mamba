...
class DiffMamba2Cache:
    ...
    def __init__(
        self, config: Mamba2Config, layers: list, batch_size: int, dtype: torch.dtype = torch.float16, device: Optional[str] = None
    ):
        self.dtype = dtype
    ...
    def update_conv_state(
        ...
        if cache_init:
            self.conv_states[layer_idx] = new_conv_state.to(self.conv_states[0].device)
        else:
...
class Mamba2Mixer(nn.Module):
    ...
    def cuda_kernels_forward(
        ...
        cache_params: Optional[DiffMamba2Cache] = None,
    ...
    # fmt: off
    def torch_forward(self, input_states, cache_params: Optional[DiffMamba2Cache]=None, cache_position:Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None):
...
def load_states(pretrained_param, current_param, new_state_dict, name, load_first):
    if name == 'conv1d.weight':
        length = current_param.shape[0]
        new_state_dict[name] = pretrained_param[:length, :, :] if load_first else pretrained_param[-length:, :, :]
    if name == 'conv1d.bias':
        length = current_param.shape[0]
        new_state_dict[name] = pretrained_param[:length] if load_first else pretrained_param[-length:]
    ...
    if name == 'out_proj.weight':
        length = current_param.shape[1]
        new_state_dict[name] = pretrained_param[:, :length] if load_first else pretrained_param[:, -length:]
    return new_state_dict


def load_partial_state_dict(current_model, pretrained_state_dict, args, load_first):
    """
    Loads a state dictionary by taking the first N parameters that fit when shapes don't match.

    Args:
        current_model: The model to load parameters into
        pretrained_state_dict: The state dictionary from the pretrained model

    Returns:
        dict: A summary of what was loaded and what was skipped
    """
    ...
    for name, pretrained_param in pretrained_state_dict.items():
        ...
        if pretrained_param.shape == current_param.shape:
            ...
            if args.verbose > 1:
                print(f"Complete load of {name}")
        else:
            new_state_dict = load_states(pretrained_param, current_param, new_state_dict, name, load_first)
            if args.verbose > 1:
                print(f"Partial load of {name} from {pretrained_param.shape} to {current_param.shape}")
...
class DiffMamba2Block(Mamba2Block):
    def __init__(
            self,
            config,
            layer_idx
        ...
        """
        Extension of Block class with DiffMamba2 specific parameters and processing.
        Inherits from the pretrained Block class and adds new functionality.
        """
        super().__init__(
            config=config,
            layer_idx=layer_idx
        ...
        # Store layer_idx and initialize DiffMamba2 specific parameters
        self.d_model = config.hidden_size

        # Initialize lambda parameters with the new scheme
    ...
    @classmethod
    def from_pretrained_block(cls, pretrained_block: Mamba2Block, args, **kwargs):
        new_block = cls(
            config=pretrained_block.config,
            layer_idx=pretrained_block.layer_idx,
            **kwargs
        )
...
MAMBA2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        cache_params (`DiffMamba2Cache`, *optional*):
...
class DiffMamba2ForCausalLM(Mamba2ForCausalLM):
    """
    Subclass of Mamba2ForCausalLM that replaces the
    last N layers with DiffMamba2Block on init.
    """
