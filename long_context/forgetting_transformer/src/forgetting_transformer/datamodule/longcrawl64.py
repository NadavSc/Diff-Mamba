...
class LongCrawl64DataModule(L.LightningDataModule):
    VOCAB_SIZE = 50257
    ...
    def __init__(
        ...
        # self.train_dataset = LongCrawlDataset(
        #     data_path=self.data_dir / "train.zarr",
        #     split="train",
        ...
        self.val_dataset = LongCrawlDataset(
            data_path=self.data_dir / "heldout.zarr",
            split="heldout",
            world_size=world_size,
            rank=rank,
    ...
    #     train_dataloader = LongCrawlDataloader(
    #         self.train_dataset,
    #         batch_size=None,
    #         shuffle=False,
    ...
    def val_dataloader(self):
        ...
        if self.eval_stateful:
            ...
            val_sampler = None
        val_dataloader = LongCrawlDataloader(
            self.val_dataset,
            batch_size=None,
            shuffle=False,
            sampler=val_sampler,
...
DOC_LEN = 65536
TRAIN_DOC_COUNT = 6609334
HELDOUT_DOC_COUNT = 52131


class LongCrawlDataset(Dataset):
    ...
    def __init__(
        ...
        DOC_COUNT = dict(train=TRAIN_DOC_COUNT, heldout=HELDOUT_DOC_COUNT)[split]
    ...
    def __getitem__(self, batch_id: int) -> Batch:
        ...
        if batch_id_in_group == 0:
            input_ids[..., 0] = 50256  # eot token from tiktoken
            resets[..., 0] = True
        else:
...
class StatefulSampler(Sampler):
    def __init__(self, dataset: LongCrawlDataset):
        super().__init__()
...
class LongCrawlDataloader(DataLoader):
