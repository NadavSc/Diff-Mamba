...
class SequenceLightningModule(pl.LightningModule):
    ...
    def forward(self, batch, output_hidden_states=False):
        ...
        x, state, hidden_states = self.model(x, **w, state=self._state)
        self._state = state
        x, w = self.decoder(x, state=state, **z)
        if output_hidden_states:
            return x, y, w, hidden_states
        return x, y, w
    ...
    def training_step(self, batch, batch_idx):
        ...
        # for i in range(18):
        #     try:
        #         print(f'{i}:{self.model.layers[i].layer._lambda}')
        #     except:
        #         pass
        # print('')
...
def create_trainer(config):
    ...
    if (config.trainer.accelerator == 'gpu' and ((isinstance(config.trainer.devices, int) and config.trainer.devices > 1)
            or (isinstance(config.trainer.devices, list) and len(config.trainer.devices) > 1))):
        print("ddp automatically configured, more than 1 gpu used!")
        config.trainer.strategy = "ddp_find_unused_parameters_true"  # previous ddp
...
def preemption_setup(config):
    ...
    #     torch.cuda.set_device(config.trainer.device_idx)
