...
def plot_softmax_values(model, indices, values, save_dir, t):
    index_symbols = model.dataset.vocab.get_symbols(indices[0])
    plt.figure(figsize=(10, 6))
    plt.bar(index_symbols, values.view(-1).cpu().numpy(), color='blue', alpha=0.7)
    plt.xticks(rotation=90)
    plt.xlabel("Symbols")
    plt.ylabel("Values")
    plt.title("Top Softmax Values")
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.savefig(os.path.join(save_dir, f'{t}_{index_symbols[0]}.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def generate(
    ...
    for t in tqdm(range(T)):
        ...
        y_t = model.step(x_t)  # [1, 214] [1, d_model]
        if len(y_t.shape) == 3:
            y_t = y_t.squeeze(0)

        # Handle special loss functions such as ProjectedAdaptiveSoftmax
        if hasattr(model.loss, "compute_logits"): y_t = model.loss.compute_logits(y_t)  # [1, 267735]

        if debug:
            y_raw.append(y_t.detach().cpu())

        # Output distribution
        probs = F.softmax(y_t, dim=-1)  # [1, 267735]
        values, indices = torch.topk(probs, 25)
        topk_probs.append(values.tolist())
        ...
        if top_p < 1.0:
            ...
            indices_to_remove[torch.arange(sorted_probs.indices.shape[0])[:, None].repeat(1, sorted_probs.indices.shape[
                1]).flatten(), sorted_probs.indices.flatten()] = csum_probs.flatten()
            y_t = y_t + indices_to_remove.int() * (-1e20)

        # Sample from the distribution
        y_t = Categorical(logits=y_t / tau).sample()  # [1]
    ...
    if not return_logprobs:
        ...
        assert not debug
        return y_all, logprobs, entropy, topk_probs, topk_indices
...
def main(config: OmegaConf):
    ...
    if config.load_data and config.prompt is None:
    ...
    for _ in range(config.n_samples // config.n_batch):
        # Construct a batch
        if config.prompt:
            indices = model.dataset.vocab.get_indices(config.prompt)
            batch = (torch.tensor(indices).to(torch.long).squeeze(1).unsqueeze(0), None, None)
            config.l_prefix = len(indices)
        ...
        _y, _logprobs, _, topk_probs, topk_indices = generate(
    ...
    if config.decode == 'audio':
        ...
        np.save(
            f'{save_dir}/unconditional_{config.dataset._name_}_{config.model._name_}_len_{config.l_sample / 16000.:.2f}s_logprobs.npy',
            logprobs)
    elif config.decode == 'text':
        y = [model.dataset.vocab.get_symbols(_y) for _y in y]
        print(f'Prediction: {"".join(y[0])}')
        with open(os.path.join(save_dir, "y.json"), "w") as file:
            json.dump(y, file)
        with open(os.path.join(save_dir, "topk_probs.json"), "w") as file:
            json.dump(topk_probs, file)
        with open(os.path.join(save_dir, "topk_indices.json"), "w") as file:
            json.dump(topk_indices, file)
