...
class Train:
    ...
    def setup(self) -> tuple[State, Union[PreTrainedModel, FSDP], int]:
        ...
        if self.dist.primary:
            ...
            data, nats_to_bpb = self.data.load(model, tokenizer, self.ssm)
        ...
        if not self.dist.primary:
            ...
            model, tokenizer = self.model.load(load_device, must_use_cache=True, ssm=self.ssm)
            data, nats_to_bpb = self.data.load(model, tokenizer, self.ssm)
            lens = self.get_lens(model)
    ...
    def execute(self):
        ...
        for batch_idx, batch in zip(t, state.dataloader):
            ...
            with th.no_grad():
                ...
                if self.ssm:
                    batch_ssm = (batch['input_ids'], batch['input_ids'], [512])
                    model._process_state(batch_ssm, batch_idx, train=True)
                    output, _, _, hidden_states = model(batch_ssm, output_hidden_states=True)
                else:
