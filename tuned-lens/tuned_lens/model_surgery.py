...
sys.path.append(os.path.join(project_path, 'tuned_lens'))
...
def get_unembedding_matrix(model: Model) -> nn.Linear:
    ...
    # if isinstance(model, SequenceLightningModule):
    #     unembed = model.decoder[1]
...
def get_final_norm(model: Model) -> Norm:
    ...
    if not hasattr(model, "base_model"):
        print("Model does not have a `base_model` attribute.")
    else:
        base_model = model.base_model

    if isinstance(model, Mamba2ForCausalLM) or isinstance(model, DiffMamba2ForCausalLM):
        base_model = model.backbone
        final_layer_norm = base_model.norm_f
    # elif isinstance(model, SequenceLightningModule):
